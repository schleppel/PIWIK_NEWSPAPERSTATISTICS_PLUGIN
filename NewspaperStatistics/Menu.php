...
class Menu extends \Piwik\Plugin\Menu
{
    public function configureReportingMenu(MenuReporting $menu)
    {
        // reuse an existing category. Execute the showList() method within the controller when menu item was clicked
         //$menu->addVisitorsItem('Report 1', $this->urlForAction('index'), $orderId = 30);
         //$menu->addActionsItem('Report 1', $this->urlForAction('index'), $orderId = 30);

        // or create a custom category 'UI Framework'
        // $menu->addItem('UI Framework', '', $this->urlForDefaultAction(), $orderId = 30);
         //$menu->addItem('UI Framework', 'Report 1', $this->urlForAction('index'), $orderId = 30);

        $menu->addItem(Piwik::translate('NewspaperStatistics_NewspaperStatistics'), Piwik::translate('NewspaperStatistics_SummaryReport'), $this->urlForAction($method = 'index'), $orderId = 30);
    }
